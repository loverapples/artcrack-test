...
//以传入节点开始遍历，直到到达所有的叶子节点 
void* scan(art_node *t){
	art_node *n = t;
	int i;
	//printf("scan\n");
	//处理叶子节点 
...
/*
			...
			uint32_t len = ((art_leaf*)n)->key_len;
			unsigned char temp[len+1];
			memcpy(temp,((art_leaf*)n)->key,len);
			temp[len] = '\0';
			//printf("%s\n",temp);
			printf("%d\n",char_to_int((unsigned char*)temp));*/
...
//单点搜索，flag字段为大于或者小于 
void* single_view_scan(art_node *t,unsigned char *key,int flag){
	//printf("SVC%c\n",*key);
	art_node *n = t;
	//int i;
	unsigned char c;
	...
	if (IS_LEAF(n)) {
		...
		if(flag == 1){
			if(mystrcmp(key,(unsigned char*)&temp[len-keylen]) > 0 );
				//printf("%d\n",char_to_int((unsigned char*)temp));
		}else{
			if(mystrcmp(key,(unsigned char*)&temp[len-keylen]) < 0 );
			//	printf("%d\n",char_to_int((unsigned char*)temp));
		...
		//printf("single 4\n");
			...
			for (int i=0 ; i < n->num_children; i++){
				if(flag == 1){
					if((p.p1->keys)[i] < *key)
						scan(p.p1->children[i]);
					else if	((p.p1->keys)[i] == *key)
						single_view_scan(p.p1->children[i],key++,flag);	
				}else{
					if((p.p1->keys)[i] > *key){
						scan(p.p1->children[i]);
					}
		...
		case NODE16:
			...
			for (int i=0 ; i < n->num_children; i++){
				if(flag == 1){
					if((p.p2->keys)[i] < *key)
						scan(p.p2->children[i]);
					else if	((p.p2->keys)[i] == *key)
						single_view_scan(p.p2->children[i],key++,flag);	
				}else{
					if((p.p2->keys)[i] > *key)
						scan(p.p2->children[i]);
					else if((p.p2->keys)[i] == *key)
...
//节点范围搜索，传入最大和最小范围 
void*  node_view_search(art_node *t,unsigned char *low,unsigned char *high){
 	art_node **child;
    art_node *n = t;
	//printf("node_view_search\n");   
    int prefix_len,i, depth = 0;
	if (IS_LEAF(n)) {
		n = (art_node*)LEAF_RAW(n);
		if(mystrcmp(((art_leaf*)n)->key,low)> 0 && mystrcmp(((art_leaf*)n)->key,high)< 0){
/*
			uint32_t len = ((art_leaf*)n)->key_len;
			char temp[len+1];
			memcpy(temp,((art_leaf*)n)->key,len);
		...
		//根据节点类型去找下一个节点 
			case NODE4:
				//printf("NVS node4\n");
 				...
 				//当部分长不为0且为cmplen长度,首先比较cmplen长度的关键字
			 	// 有如下情况：
				 //1、在范围之间，则执行扫描
				 //2、等于最小值，小于最大值，执行单侧搜索，flag为小于
				 //3、等于最大值，大于最小值，执行单侧搜索，flag为大于
				 //4、等于最大值和最小值，忽略部分长，比较keys
				 //5、在范围外 ，忽视。 
 				if(cmplen){
 				//	printf("node4有前缀\n");
				 	int lowcmp = memcmp(n->partial,low,cmplen);
				 	int highcmp = memcmp(n->partial,high,cmplen);
				 	if(lowcmp > 0 && highcmp < 0)
				 		scan(n);
			 		else if(highcmp == 0 && lowcmp > 0 ){
		 				high = high + cmplen;
		 				single_view_scan(n,high,1);
		 			}else if(lowcmp == 0 && highcmp < 0){
			 			low = low +cmplen;
			 			single_view_scan(n,low,0);
			 		}else if(lowcmp == 0 && highcmp == 0){
			 			low += cmplen;
			 			high += cmplen;
 						for (i=0 ; i < n->num_children; i++) {
							if ((p.p1->keys)[i] > *low && (p.p1->keys)[i] < *high){
     							scan(p.p1->children[i]);
							}else if((p.p1->keys)[i] == *low && (p.p1->keys)[i] < *high){
								single_view_scan(p.p1->children[i],low+1,0);	
							}else if((p.p1->keys)[i] == *high && (p.p1->keys)[i] > *low){
								single_view_scan(p.p1->children[i],high+1,1);		
							}else if((p.p1->keys)[i] == *low  && (p.p1->keys)[i] == *high ){
								node_view_search(p.p1->children[i],low+1,high+1);
							}
 						}
		 			}
				...
				//printf("NVS node16\n");
 				...
 				if(cmplen){
				 	...
				 	if(lowcmp > 0 && highcmp < 0){
			 			...
			 			high += cmplen;
 						for (i=0 ; i < n->num_children; i++) {
							if ((p.p2->keys)[i] > *low && (p.p2->keys)[i] < *high){
     							scan(p.p2->children[i]);
							}else if((p.p2->keys)[i] == *low && (p.p2->keys)[i] < *high){
								single_view_scan(p.p2->children[i],low+1,0);	
							}else if((p.p2->keys)[i] == *high && (p.p2->keys)[i] > *low ){
								single_view_scan(p.p2->children[i],high+1,1);		
							}else if((p.p2->keys)[i] == *low  && (p.p2->keys)[i] == *high){
								node_view_search(p.p2->children[i],low+1,high+1);
							}
 						}
		 			}
            		...
            		if(cmplen){
				 		...
				 		if(lowcmp > 0 && highcmp < 0){
            				...
            				for(c;c <= *high;c++){
		            			i = p.p3->keys[c];
								if (i && c == *low && c < *high)
									single_view_scan(p.p3->children[i-1],low+1,0);
								else if (i && c > *low && c == *high)
									single_view_scan(p.p3->children[i-1],high+1,1);
								else if(i && (c == *low && c == *high))
									node_view_search(p.p3->children[i-1],low+1,high+1);
								else if(i)
									scan(p.p3->children[i-1]);	
		            		}
			 			}
            			...
            			//printf("有前缀\n");
				 		...
				 		if(lowcmp > 0 && highcmp < 0){
							...
							for(c;c <= *high;c++){
		            			if(p.p4->children[c]){
									if (c == *low && c < *high)
										single_view_scan(p.p4->children[c],low+1,0);
									else if (c > *low && c == *high)
										single_view_scan(p.p4->children[c],high+1,1);
									else if(c == *low && c == *high)
										node_view_search(p.p4->children[c],low+1,high+1);
									else
										scan(p.p4->children[c]);
		            			}		
		            		}
...
void* art_view_search(const art_tree *t,unsigned char *low,
	unsigned char *high){	
	//printf("执行art查询%d,%d\n",char_to_int(low),char_to_int(high));
	art_node *n = t->root;
	//printf("view search begins\n");
	node_view_search(n,low,high);
	//printf("view search ends\n");
	return NULL;
}
