...
/*
	map<int,int>::iterator iterlow,iterhigh,it,it1;
	cout<<"range from "<<low<<" to "<<high<<endl;
	iterlow = rg.lower_bound(low);
	///cout<<"need to remove"<<endl;
	int l,h;
	l = low;
	//cout<<low<<iterlow->first<<endl;
	iterhigh = rg.upper_bound(high);
	for(it = iterlow;it != iterhigh; it++){
		h = it->first;
		if(l != it->first)
			cout<<"("<<l<<","<<h<<")"<<endl;
		l = it->second;
